...
RESUME_SNAPSHOT_EXTRACT = """你是简历结构化抽取器。根据用户与 Lisa 的对话记录，抽取当前已确认的简历信息。

规则：
- 只输出 JSON，不要 markdown 代码块，不要解释。
- 不确定的字段用空字符串 "" 或空数组 []。
- 不要编造用户未提供的信息。
- sections 中 type 只能是：education, experience, project, skill, language, certification, custom

JSON Schema 示例（字段必须齐全）：
{
  "title": "简历标题，如：前端工程师简历",
  "target_job": "目标岗位",
  "summary": "个人简介/摘要，没有则为空字符串",
  "basic_info": {
    "name": "",
    "email": "",
    "phone": "",
...
若某模块尚无信息，可将该 section 的 items 或 categories 设为空数组，但仍保留 section 占位。"""
...
class PromptManager:
    ...
    @staticmethod
    def get_resume_snapshot_extract_prompt() -> str:
        return RESUME_SNAPSHOT_EXTRACT
